...
def test_write_and_search(tmp_path):
...
def test_complete_recalls_nearest(tmp_path):
    store = NeuroStore(tmp_path / "db.npz")
...
def test_complete_with_mask_fills_unknown_fields(tmp_path):
    store = NeuroStore(tmp_path / "db.npz")
...
def test_anomaly_flags_the_off_field(tmp_path):
    store = NeuroStore(tmp_path / "db.npz")
...
def test_empty_complete_raises(tmp_path):
    store = NeuroStore(tmp_path / "db.npz")
...
def test_fields_length_validation(tmp_path):
    store = NeuroStore(tmp_path / "db.npz")
    with pytest.raises(MemoryError_):
        store.create_memory("m", 3, fields=["only", "two"])


def test_duplicate_memory_and_delete(tmp_path):
    store = NeuroStore(tmp_path / "db.npz")
...
def test_delete_pattern_reindexes(tmp_path):
    store = NeuroStore(tmp_path / "db.npz")
